...
static const std::array<std::array<unsigned char, OkFont::GLYPH_H>, 26>
    LETTER_FONT = {{
...
static const std::array<std::array<unsigned char, OkFont::GLYPH_H>, 16>
    PUNCT_32_47 = {{
...
static const std::array<std::array<unsigned char, OkFont::GLYPH_H>, 7>
    PUNCT_58_64 = {{
...
static const std::array<std::array<unsigned char, OkFont::GLYPH_H>, 6>
    PUNCT_91_96 = {{
...
static const std::array<std::array<unsigned char, OkFont::GLYPH_H>, 4>
    PUNCT_123_126 = {{
...
const unsigned char *OkFont::glyphRows(char c) {
  static const std::array<unsigned char, OkFont::GLYPH_H> BLANK = {0, 0, 0, 0,
                                                                   0, 0, 0};
  if (c >= 'a' && c <= 'z') {
    c = static_cast<char>(c - 'a' + 'A');
  }
  if (c >= '0' && c <= '9') {
    return DIGIT_FONT[c - '0'].data();
  }
  if (c >= 'A' && c <= 'Z') {
    return LETTER_FONT[c - 'A'].data();
  }
  if (c >= ' ' && c <= '/') {
    return PUNCT_32_47[c - ' '].data();
  }
  if (c >= ':' && c <= '@') {
    return PUNCT_58_64[c - ':'].data();
  }
  if (c >= '[' && c <= '`') {
    return PUNCT_91_96[c - '['].data();
  }
  if (c >= '{' && c <= '~') {
    return PUNCT_123_126[c - '{'].data();
  }
  return BLANK.data();
}
