...
Do not drive the console that way. Each key is one MCP call with its own round trip, so `hide sidewalks` costs sixteen of them and takes about as many seconds — long enough that layer toggling gets avoided during a debugging session, which is when it is worth most. Use `console` instead: one call, one line, and the command's own output comes back as the result.
