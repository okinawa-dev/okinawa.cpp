...
| `input` | Reads or sets whether the **person at the window** can drive with their own keyboard and mouse. With no arguments it reports. `enabled: false` takes the keyboard, so a stray key cannot move the view under a measurement; `enabled: true` gives it straight back. A block **expires on its own** after `seconds` (default 300, clamped to 1..3600) and can always be lifted at the window by **holding escape**; while it holds, the app says so on screen. Injected input (`press_key`, `view`) is unaffected either way. See below. |
...
- **the window always wins.** Holding `escape` for a second lifts any
  block, read straight from the device before the gate that would have
  swallowed it. Nothing running in the background can override that. It
  is a hold and not a tap, so it is never the same gesture as the
  application's own quit -- and it is one key, because a person whose
  keyboard has just gone dead is the last person able to look a chord
  up;
- **it is on screen.** A line in the corner says the input is held, how
  many seconds are left and that holding escape ends it -- and while
  escape is down it counts the hold out loud, so a release that does not
  happen can be told apart from a key that never arrived.
